...
const PORTS: [Port; 4] = [
    Port { subdev: 2, chan_offset:  0, dir: 0 },
    Port { subdev: 3, chan_offset:  0, dir: 1 },
    Port { subdev: 3, chan_offset:  8, dir: 1 },
    Port { subdev: 3, chan_offset: 16, dir: 0 },
];
...
impl ElevIo {
    ...
    pub fn write_analog(&self, channel: usize, value: usize) -> io::Result<()> {
        let ch = channel as c_uint;
        let ret = unsafe { comedi_data_write(self.it, ch >> 8, ch & 0xff, 0, 0, value as c_uint) };
        if ret == 0 { Ok(()) }
    ...
    pub fn read_analog(&self, channel: usize) -> io::Result<usize> {
        let ch = channel as c_uint;
        let mut data: c_uint = 0;
        let ret = unsafe { comedi_data_read(self.it, ch >> 8, ch & 0xff, 0, 0, &mut data as *mut c_uint) };
        if ret == 0 { Ok(data as usize) }
